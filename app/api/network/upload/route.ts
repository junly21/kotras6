...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${externalApiUrl}/selectNetWorkUpload.do`, {
      method: "POST",
    ...
    // 외부 API 응답의 success 필드를 확인하여 성공/실패 판단
    if (result.success === true) {
      ...
      // 외부 API에서 실패 응답을 보낸 경우
      return NextResponse.json(
        {
          success: false,
          error: result.message || "파일 업로드에 실패했습니다.",
        },
        {
          status: 400,
          headers: createCorsHeaders(),
