...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("노선별 정산 CSV 다운로드 API 호출됨", body);
    ...
    if (contentType && contentType.includes("text/csv")) {
      ...
      return new NextResponse(csvWithBom, {
        headers: {
          ...
          "Content-Disposition": "attachment; filename=settlement_by_route.csv",
          ...createCorsHeaders(),
        },
      });
    }
    ...
    console.error("노선별 정산 CSV 다운로드 API 처리 중 오류 발생:", error);
    return NextResponse.json(
      {
        error: "서버 내부 오류",
