...
// 검증 스키마 - PATH_KEY 입력 필드로 변경
const routeSearchKeySchema = z.object({
  PATH_KEY: z.string().min(1, "Path Key를 입력해주세요"),
});

// path_key 검색용 필터 설정
const routeSearchKeyFilterConfig = [
  {
    name: "PATH_KEY",
    label: "Path Key",
    type: "text" as const,
    placeholder: "예: 1234_13423",
    required: true,
  },
];

// 기본값
const defaultValues: RouteSearchKeyFilter = {
  PATH_KEY: "",
};

export default function RouteSearchKeyPage() {
  const gridRef = useRef<AgGridReact>(null);

  // TODO: 나중에 실제 엔드포인트로 변경 필요
  // 현재는 test 엔드포인트를 사용하지만, selectPathResultInputTex.do 엔드포인트로 변경 예정
  const { searchResults, searchLoading, searchError, handleSearch } =
    useRouteSearchKey();

  const [filters, setFilters] = useState<RouteSearchKeyFilter>(defaultValues);
  ...
  const handleFilterChange = useCallback((values: RouteSearchKeyFilter) => {
    setFilters(values);
  }, []);

  // 검색 핸들러 - useCallback으로 최적화
  const handleSearchSubmit = useCallback(
    (values: RouteSearchKeyFilter) => {
      // TODO: 실제 엔드포인트에서는 PATH_KEY를 사용하여 검색
      // 엔드포인트: selectPathResultInputTex.do
      // 파라미터: { PATH_KEY: values.PATH_KEY }
      console.log("Path Key 검색:", values.PATH_KEY);
  ...
  return (
    <ProtectedRoute requiredPath="/route-search/key-search">
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold">경로탐색 Key 검색</h1>
        </div>
        ...
        {/* 필터 폼 */}
        <FilterForm
          fields={routeSearchKeyFilterConfig}
          defaultValues={defaultValues}
          schema={routeSearchKeySchema}
          values={filters}
