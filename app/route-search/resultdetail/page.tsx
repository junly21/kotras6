...
import { processresultdetailResults } from "@/features/routeSearch/view2GridDataProcessor";
...
export default function PathKeyPage() {
  ...
  const processedResults = useMemo(() => {
    return processresultdetailResults(searchResults, nodes);
  }, [searchResults, nodes]);
  ...
  const getRowStyle = useCallback(
    (params: {
      ...
      // 선택된 행 하이라이트 - path_key + path_seq 조합으로 정확하게 비교
      if (
        params.data &&
        selectedRow &&
        isSameRoute(params.data.originalData, selectedRow)
      ) {
  ...
  return (
    <ProtectedRoute requiredPath="/route-search/resultdetail">
