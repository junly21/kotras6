import { RouteSearchTestResult2 } from "@/types/routeSearch";
...
interface RouteSearchTestGridData {
  ...
  detailedPath: string;
  isSelected: boolean;
  originalData: RouteSearchTestResult2;
  cnt: number;
  old_cnt: number;
}

export function processRouteSearchTestResults(
  searchResults: RouteSearchTestResult2[],
  selectedPaths: RouteSearchTestResult2[]
  ...
  return sortedResults.map((result, index) => {
    ...
    return {
      ...
      groupDisplay2: isFirstInGroup ? result.old_path_key : null,
      ...
      old_cnt: result.old_cnt || 0, // API에서 cnt 필드가 추가될 예정
    };
  });
}
